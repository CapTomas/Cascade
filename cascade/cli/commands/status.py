...
def _display_recommendations(tickets: dict) -> None:
    ...
    if tickets["in_progress"] > 0:
        ...
        content.append(f"[accent]›[/accent] Run [white]cascade ticket execute {active.id}[/white]", style="muted")
        ...
        content.append(f"[accent]›[/accent] Run [white]cascade ticket execute {ready.id}[/white]", style="muted")
        ...
        if defined_count > 0:
            ...
            content.append("[accent]›[/accent] Run [white]cascade ticket ready <id>[/white] to activate", style="muted")
        else:
